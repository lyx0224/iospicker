...
import iospicker.lib.adapter.NumericWheelAdapter;
...
public class WheelTime
{
	...
	public void setPicker(int year, int month, int day, int h, int m)
	{
		...
		wv_year.setAdapter(new NumericWheelAdapter(startYear, endYear, "%02d"));// 设置"年"的显示数据
		wv_year.setLabel(context.getString(R.string.pickerview_year));// 添加文字
		wv_year.setCurrentItem(year - startYear);// 初始化时显示的数据

		// 月
		wv_month = (WheelView) view.findViewById(R.id.month);
		wv_month.setAdapter(new NumericWheelAdapter(1, 12, null));
		...
		{
			wv_day.setAdapter(new NumericWheelAdapter(1, 31, "%01d"));
		} else if (list_little.contains(String.valueOf(month + 1)))
		{
			wv_day.setAdapter(new NumericWheelAdapter(1, 30, "%01d"));
		} else
		{
			// 闰年
			if ((year % 4 == 0 && year % 100 != 0) || year % 400 == 0)
				wv_day.setAdapter(new NumericWheelAdapter(1, 29, "%01d"));
			else
				wv_day.setAdapter(new NumericWheelAdapter(1, 28, "%01d"));
		}
		...
		wv_year.addOnItemSelectedListener(wheelListener_year);
		wv_month.addOnItemSelectedListener(wheelListener_month);
	...
	public void setWheelListener(final TimePickerView.PickerListener listener){
		if (listener == null) return;
		wv_year.addOnItemSelectedListener(new OnItemSelectedListener() {
			@Override
			public void onItemSelected(int index) {
				listener.onGetCurrent(wv_year.getCurrentItem(),wv_month.getCurrentItem(), wv_day.getCurrentItem());
			}
		});
		wv_month.addOnItemSelectedListener(new OnItemSelectedListener() {
			@Override
			public void onItemSelected(int index) {
				listener.onGetCurrent(wv_year.getCurrentItem(),wv_month.getCurrentItem(), wv_day.getCurrentItem());
			}
		});
		wv_day.addOnItemSelectedListener(new OnItemSelectedListener() {
			@Override
			public void onItemSelected(int index) {
				listener.onGetCurrent(wv_year.getCurrentItem(),wv_month.getCurrentItem(), wv_day.getCurrentItem());
			}
